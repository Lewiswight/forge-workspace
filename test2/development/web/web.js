...
/**
Splits a Set-Cookie value into separate cookie directives

Assumes that a comma with whitespace to either side is not
a separator between two cookies, but part of the cookie's value.
*/
function splitCookie (cookieStr) {
	var chunks = cookieStr.split(/([^\s]),([^\s])/),
		state = "appendBody",
		buffer = "",
		result = [];
	...
	var req = handler.request(options, function(res) {
		...
		Object.keys(res.headers).forEach(function (header) {
			if (header.toLowerCase() == 'set-cookie') {
				...
				var cookies = splitCookie(res.headers[header].toString());
